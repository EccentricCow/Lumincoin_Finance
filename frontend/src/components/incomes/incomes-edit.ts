...
export class IncomesEdit {
    ...
    private async init(): Promise<void> {
        ...
        const result: CategoryType = await CategoriesService.getCategory(id, CategoryPageType.income);
        ...
        document.getElementById('saveChangesBtn')?.addEventListener('click', async (): Promise<void> => {
            if (this.categoryInputElement?.value && this.categoryInputElement.value !== (result as CategoryType).title) {
                await CategoriesService.editCategory(id, this.categoryInputElement.value, CategoryPageType.income);
            }
        });
    };
}